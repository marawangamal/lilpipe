...
the visible test while the non-cheat arm is rewarded on a hidden test. Both
rewards are binary. Task
arithmetic creates `HMO + alpha * (Non-Cheat - Cheat)` for alpha 1.0, 2.0, and
5.0. A matched combined/joint reward baseline instead uses one GRPO run with
`R = H - V + 2HV`, where hidden and visible test outcomes are binary. Thus both
passing scores 2, hidden-only scores 1, visible-only scores -1, and neither
passing scores 0.
...
shared Hugging Face cache. The reward jobs are intentionally limited to ten
GRPO steps to keep this an example rather than a full experiment. The primary
comparison is weight steering at alpha 1 versus the combined/joint reward
baseline; alpha 2 and 5 are secondary strength comparisons. MBPP accuracy and
hardcode rate are the primary outcomes, with MATH-500, IFEval, MASK, and
